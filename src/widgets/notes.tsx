...
export const Notes = () => {
  ...
  const getEntries = async () => {
    try {
      ...
      await entryStore.setPath(folderIds);
  ...
  const handleCreateFolder = async () => {
    ...
    if (label) {
      await entryStore.createFolder(label);
    }
  };
    ...
    <div className="flex flex-col sticky top-0 bg-background z-10">
      <div className="flex justify-between items-center py-4">
        <div className="flex items-center gap-2">
          {entryStore.pathIds.length > 0 && (
            <Button size="icon" variant="outline" onClick={handleGoBack}>
              <ArrowLeft />
            </Button>
          )}
          <h2 className="text-lg md:text-xl font-bold">
            {entryStore.dir ? entryStore.dir : t("notes.title")}
          </h2>
        </div>
  ...
  const renderContent = () => {
    ...
    if (!entryStore.entries.length) {
      return <p className="text-muted-foreground">{t("notes.empty")}</p>;
    }

    return (
      <div className="grid gap-4">
        <EntryList entries={entryStore.entries} onCdInto={handleCdInto} />
      </div>
    );
  };
